...
public class PlayerScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        puzzleManager = gm.GetComponent<PuzzleManagerScript>();
    }

    void Update()
    {
        GetInput();	// VAI SER REALOCADO PARA UM SCRIPT SEPARADO
    ...
    void Use()
    {
        ...
        if (objUsed.GetComponent<ObjectScript>().objectType == Enum.Type.npc) StartDialogue(objUsed);

    }

    void StartDialogue(GameObject objUsed)
    {
        print("dialogou");
        NPCScript currentNPC = objUsed.GetComponent<NPCScript>();
        FindObjectOfType<DialogueManager>().StartDialogue(currentNPC.dialogue);
    }
